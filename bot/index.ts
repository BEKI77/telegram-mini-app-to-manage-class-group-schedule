...
bot.command("start", async (ctx) => {
  ...
  if (ctx.chat.type === "private") {
    ctx.reply(
      "Welcome! To create a classroom, add me to a Telegram Group and run /create <name> inside the topic you want to use."
    );
  } else {
    ctx.reply(
      "Hi! Use /create <Classroom Name> to turn this topic into a classroom."
    );
  }
});
